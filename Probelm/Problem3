...
def Connection():
    ...
    data = [{'pubDate': '2020-05-06 13:30:07',
      'channelName': '财经最新',
    ...
    for i, x in enumerate(data):
        print("Data",x)
        newsdata.insert_one(x)
        with open('%d.txt'%i, mode='w') as file_hanle:
            file_hanle.write(x)
        np.save('%d.npy'%i, x, allow_pickle=True)
    # for i in range(3):
    #
    #     x = np.load('%d.npy' % i, allow_pickle=True)
    #     print(x)
